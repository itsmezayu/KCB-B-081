...
map<string, vector<string>> romania = {
...
vector<string> bfs(string start, string goal)
{
    ...
    while (!q.empty())
    {
        ...
        for (const string &neighbor : romania[node])
        {
            if (!visited[neighbor])
            {
...
vector<string> dfs(string start, string goal, map<string, bool> &visited, vector<string> path)
{
    ...
    for (const string &neighbor : romania[start])
    {
        if (!visited[neighbor])
        {
...
void arrowSymbols(const vector<string> &route)
{
...
int main()
{
    ...
    {
        cout << "Metode invalid! Pilih BFS atau DFS." << endl;
    }
